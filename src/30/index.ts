function getRandomInt(scope: number): number {
...
class WhackMole {
    ...
    #scoreEl;
    #score = 0;
    #gameId: ReturnType<typeof setTimeout> | undefined

    constructor() {
        this.#holeElList = document.querySelectorAll('.hole') as NodeListOf<HTMLDivElement>;
        this.#scoreEl = document.querySelector('.score') as HTMLSpanElement;
    ...
    triggerMole() {
        ...
        list.forEach((item, index) => {
            const target = this.#holeElList[item].querySelector('.mole') as HTMLDivElement;
    ...
    resetScore() {
        if (this.#gameId) {
            ...
            this.#holeElList.forEach((item) => {
                (item.querySelector('.mole') as HTMLDivElement).style.top = '100%';
            })
        }
    ...
    listener() {
        document.querySelector('.game')?.addEventListener('click', (e) => {
            ...
            const target = (e.target as HTMLDivElement).closest('.mole') as HTMLDivElement;
            if (!target) return;
            target.style.pointerEvents = 'none';
            target.style.top = '100%';
            this.#score ++;
