...
class CityList {
    #cityList: CityList[] = [];
    #searchedList: CityList[] = [];
    #keyword: string | null = null;
    #inputEl;
    #outputEl;

    constructor(api: string, options: RequestInit = {}, inputEl: Element | null, outputEl: Element | null) {
    ...
    async fetchData(api: string, options: RequestInit) {
        try {
            const res = await fetch(api, options);
            const data = await res.json() as CityList[];
            this.#cityList = [...data];
