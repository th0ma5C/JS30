...
class ArrTool {
    ...
    anyAbove19(arr: typeof guys) {
        return arr.some(item => (this.#year - item.year) >= 19)
    }

    everyAbove19(arr: typeof guys) {
        return arr.every(item => (this.#year - item.year) >= 19)
    }

    findID_823423(arr: typeof comments) {
        return arr.find(item => {
            if (item.id === 823423) return item.text
        })
    }

    deleteCommentWithID_823423(arr: typeof comments) {
        const ans = [...arr];
